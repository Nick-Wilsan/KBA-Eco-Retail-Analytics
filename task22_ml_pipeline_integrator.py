...
def run_task20(demo: bool, data_dir: str, store: str, db: str | None):
    """Jalankan Task 20 Prophet Demand Forecast."""
    print("\n" + "▶" * 3 + "  TASK 20 — Prophet Demand Forecast")

    import importlib.util, pathlib
    spec = importlib.util.spec_from_file_location(
        "task20", pathlib.Path(__file__).parent / "task20_prophet_demand_forecast.py"
    )
...
def run_task21(demo: bool, data_dir: str, db: str | None):
    """Jalankan Task 21 Isolation Forest Cold Chain."""
    print("\n" + "▶" * 3 + "  TASK 21 — Isolation Forest Cold Chain")
    import importlib.util, pathlib
    spec = importlib.util.spec_from_file_location(
        "task21", pathlib.Path(__file__).parent / "task21_isolation_forest_coldchain.py"
    )
...
def build_executive_kpi(
    ...
    """
    Bangun gold_mart_executive_kpi — satu baris per toko per tanggal snapshot.
    Metrik yang ada di sini akan langsung ditampilkan di Executive Dashboard Roeney (Metabase).
    PRD §2.1: VP, Direktur, GM — format ringkas, kode warna.
    """
...
def build_alert_table(df_fc: pd.DataFrame, df_com: pd.DataFrame) -> pd.DataFrame:
    """
    Gabungkan forecast + compliance → tabel alert SKU yang butuh perhatian.
    PRD §2.2: Analyst Dashboard — drill-down per SKU.
    """
    # Agregat forecast per SKU
    fc_agg = (
...
def generate_report(df_kpi: pd.DataFrame, df_alert: pd.DataFrame) -> str:
    ...
    A. DEMAND FORECAST — Prophet (Task 20)
  ]
    ...
    B. COLD CHAIN — Isolation Forest (Task 21)
    ...
    E. OUTPUT FILES 
    • output/demand_forecast_output.csv         → Pipeline Zain (Prefect → dbt → DuckDB)
    • output/anomaly_coldchain_output.csv       → Pipeline Zain
    • output/coldchain_compliance_summary.csv   → Analyst Dashboard Roeney
    • output/mape_walkforward_summary.csv       → QA Kresna
    • DuckDB gold_mart_demand_forecast          → Metabase Roeney
    • DuckDB gold_mart_cold_chain_compliance    → Metabase Roeney
    • DuckDB gold_mart_executive_kpi            → Executive Dashboard Roeney
    • DuckDB gold_mart_sku_alert                → Analyst Dashboard Roeney
    • output/isolation_forest_model.pkl         → Inference oleh Prefect pipeline Zain
...
def run_ml_pipeline(
    ...
    """
    Entry point untuk Prefect flow (Zain).

    Contoh pemakaian di Prefect:
        from task22_ml_pipeline_integrator import run_ml_pipeline

        @flow(name="eco-retail-ml-pipeline")
        def ml_flow():
            result = run_ml_pipeline(demo=False, db_path="eco_retail.duckdb")
            return result
    """
    if run_models:
