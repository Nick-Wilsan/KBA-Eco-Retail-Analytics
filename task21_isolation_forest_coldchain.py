...
def load_coldchain_data(data_dir: str, store_id: str, n_sku: int) -> pd.DataFrame:
    """
    Load Cold Chain Monitoring Dataset dari Kaggle/UCI.
    Ekspektasi kolom minimal:
        timestamp, device_id/sku_id, temperature, humidity, [door_open_sec]
    Sesuaikan nama kolom di bawah jika berbeda.
    """
...
def generate_demo_coldchain(store_id: str, n_sku: int, n_per_sku: int = 2000) -> pd.DataFrame:
    """
    Data sensor sintetis yang meniru Cold Chain Monitoring Dataset (Kaggle/UCI).
    Mengikuti PRD §4.4: equipment_breach (suhu naik gradual >30 min)
                       vs operational_error (lonjakan sementara).
    """
...
def classify_anomaly_type(row) -> str:
    """
    Bedakan equipment_breach vs operational_error (PRD §4.4).
    - equipment_breach : drift gradual (temp_cumsum_3 tinggi atau door lama terbuka)
    - operational_error: spike mendadak (temp_delta_abs tinggi tapi roll_std rendah)
    """
...
def compute_compliance(df: pd.DataFrame) -> pd.DataFrame:
    """
    Hitung compliance rate per SKU dan per zona.
    Compliance = pembacaan NORMAL / total pembacaan × 100%
    Target PRD §5: ≥ 95%
    """
    summary = (
    ...
    # Tipe anomali dominan per SKU (equipment_breach vs operational_error vs normal)
