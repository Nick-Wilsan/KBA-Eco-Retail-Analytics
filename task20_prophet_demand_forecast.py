...
def train_and_forecast(df_sku: pd.DataFrame, sku_id: str) -> tuple[pd.DataFrame, float]:
    ...
    result.columns = ["forecast_date", "forecast_qty", "lower_bound_80", "upper_bound_80"]
    result["forecast_qty"]   = np.maximum(0, result["forecast_qty"]).round().astype(int)
    result["lower_bound_80"] = np.maximum(0, result["lower_bound_80"]).round().astype(int)
    result["upper_bound_80"] = np.maximum(0, result["upper_bound_80"]).round().astype(int)
    result["sku_id"]         = sku_id
    result["mape_pct"]       = round(mape, 2)
    result["mape_ok"]        = mape <= CFG["mape_target"]

    result["mape_contribution"] = round(mape / CFG["mape_target"] * 100, 2)
...
def save_to_duckdb(df: pd.DataFrame, db_path: str):
    ...
    CREATE TABLE IF NOT EXISTS {CFG['gold_table']} (
        ...
        lower_bound_80       INTEGER,
        upper_bound_80       INTEGER,
        mape_pct             DOUBLE,
        mape_ok              BOOLEAN,
        mape_contribution    DOUBLE,
...
def main(args):
    ...
    "forecast_qty", "lower_bound_80", "upper_bound_80",
    "mape_pct", "mape_ok", "mape_contribution",
    ]]
